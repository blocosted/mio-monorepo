...
async function createPostgresContainer(containerName: string, config: TestEnvConfig): Promise<void> {
  ...
  while (retries < maxRetries) {
    try {
      execSync(`docker exec ${containerName} pg_isready -U ${config.databaseUser}`, { stdio: 'ignore' });
      break;
    } catch {
      retries++;
...
async function createRedisContainer(containerName: string, config: TestEnvConfig): Promise<void> {
  ...
  while (retries < maxRetries) {
    try {
      execSync(`docker exec ${containerName} redis-cli -a ${config.redisPassword} ping`, { stdio: 'ignore' });
      break;
    } catch {
      retries++;
      if (retries >= maxRetries) {
 ...
 */
export async function setupDatabase(containerName: string, config: TestEnvConfig = DEFAULT_TEST_CONFIG): Promise<string> {
 ...
 */
export async function setupRedis(containerName: string, config: TestEnvConfig = DEFAULT_TEST_CONFIG): Promise<string> {
 ...
 */
export async function cleanTestRedisData(containerName: string, password: string): Promise<void> {
  try {
    execSync(`docker exec ${containerName} redis-cli -a ${password} FLUSHALL`, { stdio: 'ignore' });
  } catch (error) {
