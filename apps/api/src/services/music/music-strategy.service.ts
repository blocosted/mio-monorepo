...
const DEFAULT_PUNCTUAL_CONFIG: PunctualStrategyConfig = {
  intro: {
    durationRatio: 0.1, // 10% of story for intro music
    mood: 'mysterious',
    fadeIn: 1.0,
    fadeOut: 2.0,
    volume: 0.15
  ...
  climax: {
    positionRatio: 0.7, // Climax around 70% of story
  ...
  outro: {
    positionRatio: 0.9, // Outro starts at 90%
    durationRatio: 0.1, // 10% of story for outro
    mood: 'calm',
    fadeIn: 1.0,
    fadeOut: 3.0,
    volume: 0.12
...
export class MusicStrategyService {
   ...
   */
  private generatePunctualCues(totalDuration: number, storyMoments?: MusicStrategyInput['storyMoments']): MusicStrategyOutput {
    const cues: MusicCue[] = [];
    ...
    const climaxPosition = storyMoments?.find((m) => m.type === 'climax')?.time ?? totalDuration * this.config.climax.positionRatio;
    ...
    const outroStart = storyMoments?.find((m) => m.type === 'resolution')?.time ?? totalDuration * this.config.outro.positionRatio;
    const outroDuration = Math.min(totalDuration * this.config.outro.durationRatio, totalDuration - outroStart);
    cues.push({
   ...
   */
  private generateFromScriptSegments(segments: Array<{ startTime: number; duration: number; mood: string }>, totalDuration: number): MusicStrategyOutput {
   ...
   */
  private generateContinuousCues(totalDuration: number, existingSegments?: Array<{ startTime: number; duration: number; mood: string }>): MusicStrategyOutput {
  ...
  convertScriptMusicToCues(
    ...
    return segments.map((seg, index) => ({
      ...
      volume: 0.15,
      reason: index === 0 ? 'intro' : index === segments.length - 1 ? 'outro' : 'transition'
    }));
  }
