...
export class StorySegmentsStore {
  ...
  async findByStoryId(storyId: string): Promise<StorySegmentRow[]> {
    return this.db.select().from(storySegments).where(eq(storySegments.storyId, storyId)).orderBy(asc(storySegments.order));
  }
  ...
  async findById(id: string): Promise<StorySegmentRow | null> {
    const [segment] = await this.db.select().from(storySegments).where(eq(storySegments.id, id)).limit(1);
  ...
  async delete(id: string): Promise<void> {
    await this.db.delete(storySegments).where(eq(storySegments.id, id));
  }
  ...
  async deleteByStoryId(storyId: string): Promise<void> {
    await this.db.delete(storySegments).where(eq(storySegments.storyId, storyId));
  }
  ...
  async findByStoryIdAndType(storyId: string, type: SegmentType): Promise<StorySegmentRow[]> {
    return this.db
      .select()
      .from(storySegments)
      .where(and(eq(storySegments.storyId, storyId), eq(storySegments.type, type)))
