 ...
 */
@injectable()
export class ElevenLabsProvider implements IElevenLabsProvider {
     ...
     */
    async convertWithTimestamps(input: ElevenLabsConvertInput): Promise<ElevenLabsConvertResult> {
        const {
            text,
            voiceId,
        ...
        // Console log for debugging (visible in CLI scripts)
        console.log(`    [API] voice_settings sent to ElevenLabs:`, JSON.stringify(apiVoiceSettings));
    ...
    async isValidVoice(voiceId: string): Promise<boolean> {
        try {
            ...
            const voiceRegistry = getInstance<import('./voice-registry.service.types').IVoiceRegistryService>(
