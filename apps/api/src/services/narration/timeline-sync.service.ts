...
export class TimelineSyncService implements ITimelineSyncService {
    ...
    buildVoiceTimeline(
        ...
        pauseBetweenSegments: number = DEFAULT_PAUSE_BETWEEN_SEGMENTS,
    ...
    syncTimings(
        ...
        // Calculate drift
        const driftPercentage = originalTotalDuration > 0
            ? Math.round(((actualTotalDuration - originalTotalDuration) / originalTotalDuration) * 100)
            : 0;
    ...
    private syncNonVoiceSegment(
        ...
        if (!voiceTrack) {
            ...
            const scaleFactor = originalTotalDuration > 0
                ? actualTotalDuration / originalTotalDuration
                : 1;
            return {
                ...segment,
        ...
        if (preserveRelative) {
            ...
            if (anchorResult) {
                ...
                if (anchorTiming) {
                    ...
                    const offsetRatio = originalAnchorDuration > 0
                        ? offsetInAnchor / originalAnchorDuration
                        : 0;
     ...
     */
    private findAnchorVoiceSegment(
        targetTime: number,
        voiceSegments: TimelineSegment[],
    ): { anchorSegment: TimelineSegment; offsetInAnchor: number } | null {
        ...
        if (closestSegment) {
            ...
            const offset = targetTime < closestSegment.startTime
                ? 0
