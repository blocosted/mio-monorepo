...
export class AmbianceGeneratorService {
  ...
  async generate(input: AmbianceGenerateInput): Promise<AmbianceGenerateResult> {
    ...
    const processedAudio = await this.processAmbianceWithFFmpeg(sourceAudio, targetDurationSeconds, fadeInDuration, fadeOutDuration, volume, needsLoop);
  ...
  private buildAmbiancePrompt(description: string): string {
    ...
    const lowerDesc = description.toLowerCase();
    const hasAmbientHints = ['ambient', 'background', 'continuous', 'looping'].some((hint) => lowerDesc.includes(hint));
