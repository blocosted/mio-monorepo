...
export class FFmpegMixerService implements IFFmpegMixerService {
    ...
    async mixStory(input: MixStoryInput): Promise<MixStoryResult> {
        ...
        try {
            ...
            const voiceTimelinePath = await this.createVoiceTimeline(
                voice,
                downloadedFiles.voice,
                workdir
            );
            ...
            const mixedPath = await this.mixAllTracks(
                voiceTimelinePath,
                voiceDuration,
                downloadedFiles,
                { music, ambiance, sfx },
            ...
            const finalPath = await this.exportFinalMix(
                normalizedPath,
                workdir,
                output
            );
    ...
    private async createWorkdir(storyId: string): Promise<string> {
        const workdir = join(
            TEMP_DIR_BASE,
            `${WORKDIR_PREFIX}-${storyId}-${randomUUID().slice(0, 8)}`
        );
     ...
     */
    private async createVoiceTimeline(
    ...
    private async mixAllTracks(
        ...
        return new Promise((resolve, reject) => {
            ...
            if (music && downloadedFiles.music) {
                ...
                if (music.enableDucking) {
                    ...
                    filterParts.push(
                        ...
                        `[music_vol][voice_sc]${FILTER_TEMPLATES.sidechainCompress(
                            d.threshold,
                            d.ratio,
                            d.attackMs,
                            d.releaseMs
            ...
            if (ambiance && downloadedFiles.ambiance) {
                ...
                if (ambiance.loop) {
                    filterParts.push(
                        `[${inputIndex}:a]${FILTER_TEMPLATES.aloop()},atrim=0:${voiceDuration},${FILTER_TEMPLATES.volume(
                            ambianceVolume
                        )}[ambiance_loop]`
                    );
                } else {
                    filterParts.push(
                        `[${inputIndex}:a]${FILTER_TEMPLATES.volume(ambianceVolume)}[ambiance_loop]`
                    );
                }
            ...
            if (sfx && sfx.files.length > 0 && downloadedFiles.sfx.length > 0) {
                ...
                sfx.files.forEach((file, i) => {
                    ...
                    if (sfxPath) {
                        ...
                        filterParts.push(
                            `[${inputIndex}:a]${FILTER_TEMPLATES.adelay(delayMs)},${FILTER_TEMPLATES.volume(
                                fileVolume
                            )}[sfx_${i}]`
                        );
     ...
     */
    private async exportFinalMix(
        inputPath: string,
        workdir: string,
        outputConfig?: MixStoryInput['output']
    ): Promise<string> {
