...
| T0102 ✔spécifiée | Étalonner la charte | Mesurer des références commerciales, comparer aux seuils actuels, réviser et justifier les écarts. | S |
| T0103 ✔spécifiée | Resserrer la génération hors workflow | Le chemin existe (`packages/scripts/src/pipeline/full-story.ts`) mais instancie les services par `new`, hors conteneur — il diverge donc du chemin de production. Le faire passer par le conteneur et le ramener à l'échelle d'une scène. | M |
| T0104 ✔spécifiée | Compléter le magasin de runs | Les stems sont déjà écrits par étape ; ajouter le script utilisé et les mesures sous le même identifiant de rendu. | S |
| T0105 ✔spécifiée | Comparer deux rendus | Écart mesure par mesure entre deux rendus, et fiche de comparaison. | S |
| T0106 ✔spécifiée | Compter le coût | Enregistrer appels d'API, caractères consommés et durée pour chaque génération. | S |
