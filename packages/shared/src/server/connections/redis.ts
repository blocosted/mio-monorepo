...
export function buildRedisUrl(config: { host: string; port: number; password?: string; db?: number }): string {
...
export class RedisClient implements IRedisClient {
  ...
  constructor(config: RedisConfig, logger?: Logger) {
    ...
    if (!url) {
      throw new Error('REDIS_URL (or REDIS_HOST + REDIS_PORT) must be provided');
    }
  ...
  public async set<T>(key: string, value: T, options: RedisSetOptions = {}): Promise<void> {
    return await this.withReconnect(async () => {
  ...
  public duplicate(): IRedisClient {
    ...
    return new RedisClient(config, this.logger ? ({ withModule: () => this.logger } as unknown as Logger) : undefined);
  }
}
