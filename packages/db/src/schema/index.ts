...
export const storySegments = pgTable('story_segments', {
  ...
  type: text('type', {
    enum: [SegmentType.Narration, SegmentType.Dialogue, SegmentType.Pause, SegmentType.SoundEffect, SegmentType.MusicChange]
  }).notNull(),
...
export const audioAssets = pgTable('audio_assets', {
  ...
  type: text('type', {
    enum: [AudioAssetType.Voice, AudioAssetType.Sfx, AudioAssetType.Music, AudioAssetType.Ambiance, AudioAssetType.FinalMix]
  }).notNull(),
...
export const generationJobs = pgTable('generation_jobs', {
  ...
  status: text('status', {
    enum: [JobStatus.Pending, JobStatus.Processing, JobStatus.Completed, JobStatus.Failed, JobStatus.Cancelled]
  })
...
export const storiesRelations = relations(stories, ({ one, many }) => ({
  ...
  computedTimeline: one(computedTimelines, {
    fields: [stories.id],
    references: [computedTimelines.storyId]
  })
}));
...
// =============================================================================
// Computed Timelines Table
// =============================================================================

/**
 * Computed Timelines Table
 *
 * Stores the computed timeline with absolute times for a story.
 * Generated by TimelineComputationService after TTS produces real durations.
 *
 * Used for:
 * - FFmpeg mixing input
 ...
 */
export const computedTimelines = pgTable('computed_timelines', {
  id: uuid('id').primaryKey().defaultRandom(),
  storyId: uuid('story_id')
    .notNull()
    .references(() => stories.id, { onDelete: 'cascade' })
    .unique(),
  /** Full computed timeline JSON */
  timeline: jsonb('timeline').$type<ComputedTimeline>().notNull(),
  /** Total duration in seconds (denormalized for queries) */
...
/**
 * Computed Timelines Relations
 */
export const computedTimelinesRelations = relations(computedTimelines, ({ one }) => ({
  story: one(stories, {
    fields: [computedTimelines.storyId],
    references: [stories.id]
  })
}));
