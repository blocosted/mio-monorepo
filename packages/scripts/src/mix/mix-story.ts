...
export async function runMixStoryCommand(args: MixStoryCommandArgs): Promise<void> {
  ...
  const voiceSegments = successfulResults
    .filter((result): result is typeof result & { outputFile: string } => result.outputFile !== undefined)
    .map((result) => {
  ...
  const totalVoiceDuration = voiceSegments.reduce((sum, s) => sum + s.duration, 0) + (voiceSegments.length - 1) * pauseDuration;
  ...
  if (args.autoMusic) {
    ...
    const _availableMoods = Object.keys(musicLibrary);
  ...
  const _ffmpegInfo = await mixerService.verifyFFmpegInstalled();
