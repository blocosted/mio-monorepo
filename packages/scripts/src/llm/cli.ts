  ...
  .command(
    ...
    (y) =>
      y
        .option('inputFile', {
          type: 'string',
          description: 'Path to a previously saved input.json (overrides --prompt/--profile)'
        })
        .option('prompt', {
          type: 'string',
        ...
        .option('envFile', {
          type: 'string',
          description: 'Optional dotenv file to load (default: tries .env.local then .env if present)'
        })
        .option('provider', {
          type: 'string',
    ...
    (y) =>
      y
        .option('enrichInputFile', {
          type: 'string',
          description: 'Path to input.json or output.json from a previous enrich-story run'
        })
        .option('inputFile', {
          type: 'string',
          description: 'Path to a previously saved generate-script input.json (alternative to enrichInputFile)'
        })
        .option('profile', {
          type: 'string',
        ...
        .option('answers', {
          type: 'string',
          description: 'JSON array of guided answers, e.g. \'[{"questionId":"ending","value":"happy"}]\''
        })
        ...
        .check((argv) => {
          if (!argv.enrichInputFile && !argv.inputFile) {
            throw new Error('Either --enrichInputFile or --inputFile is required');
          }
          return true;
        }),
    async (argv) => {
