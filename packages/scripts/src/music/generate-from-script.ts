...
import type { StoryScript, MusicSegmentContent } from '@mio/shared/models';
...
export async function runGenerateFromScriptCommand(args: GenerateFromScriptCommandArgs): Promise<void> {
    ...
    if (musicSegments.length === 0 && args.autoMusic) {
        console.log('No music segments in script. Generating automatic music cues...');
        ...
        console.log(`Generated ${musicSegments.length} automatic music cues (${strategyResult.strategy} strategy)`);
        console.log(`Coverage: ${strategyResult.coveragePercentage.toFixed(1)}%`);
        console.log('');
    }

    if (musicSegments.length === 0) {
        console.log('No music segments found in script. Use --autoMusic to generate automatic cues.');
        return;
    }
    ...
    console.log(`Found ${musicSegments.length} music segments in script "${script.metadata.title}"`);
    console.log('');

    for (const seg of musicSegments) {
        console.log(`  [${seg.id}] ${seg.mood}`);
    ...
    if (args.dryRun) {
        ...
        console.log('[DRY RUN] Music generation parameters:');
    ...
    for (const segment of musicSegments) {
        try {
            ...
            console.log(`  [OK] ${segment.id} (${result.durationSeconds.toFixed(2)}s${result.looped ? ', looped' : ''}) - ${segment.mood}`);
        } catch (error) {
            results.push({
                id: segment.id,
                success: false,
