...
export async function runSeedMusicCommand(options: SeedMusicCommandOptions): Promise<void> {
    ...
    if (options.tempo) console.log(`Filter - Tempo: ${options.tempo}`);
    ...
    if (options.dryRun) {
        ...
        for (const item of itemsToSeed) {
            console.log(`  [${item.mood}] ${item.intensity}/${item.tempo} (variation ${item.variationIndex})`);
            console.log(`    Prompt: "${item.prompt.substring(0, 70)}${item.prompt.length > 70 ? '...' : ''}"`);
            console.log('');
        }
