...
export async function createCliServices(logger: Logger): Promise<CliServices> {
  ...
  return {
    async generateSfx(input) {
      const { text, category, subcategory = 'general', environment, intensity = 'medium', durationSeconds = 5, promptInfluence = 0.3 } = input;
      ...
      if (category) {
        try {
          ...
          const canonicalParts = ['sfx', category, subcategory ?? 'general', environment ?? 'any', intensity ?? 'any', Bun.hash(text).toString(36)];
    ...
    async generateAmbiance(input) {
      const { description, environment, subEnvironment, timeOfDay = 'any', weather = 'any', mood, targetDurationSeconds, promptInfluence = 0.3 } = input;
      ...
      if (environment) {
        try {
          // Generate canonical key
          const canonicalParts = ['ambiance', environment, timeOfDay ?? 'any', weather ?? 'any', mood ?? 'any', Bun.hash(description).toString(36)];
          const canonicalKey = canonicalParts.join(':');
    ...
    async generateMusic(input) {
      const { mood, intensity = 'medium', tempo = 'medium', variationIndex = 0, customPrompt, targetDurationSeconds, promptInfluence = 0.5 } = input;
      ...
      try {
        // Generate canonical key
        const canonicalParts = ['music', mood, intensity ?? 'any', tempo ?? 'any', variationIndex.toString(), Bun.hash(prompt).toString(36)];
        const canonicalKey = canonicalParts.join(':');
    ...
    async getStats() {
      const [sfxStats, ambianceStats, musicStats] = await Promise.all([sfxStore.getStats(), ambianceStore.getStats(), musicStore.getStats()]);
