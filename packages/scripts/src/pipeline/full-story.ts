...
export async function runFullStoryCommand(args: FullStoryCommandArgs): Promise<void> {
    ...
    console.log(`FULL STORY PIPELINE: "${script.metadata.title}"`);
    console.log('='.repeat(60));
    console.log(`\nScript analysis:`);
    console.log(`  Voice segments: ${analysis.voiceCount}`);
    console.log(`  SFX segments: ${analysis.sfxCount}`);
    console.log(`  Ambiance segments: ${analysis.ambianceCount}`);
    console.log(`  Music segments: ${analysis.musicCount}`);
    ...
    const printSteps = () => {
        ...
        for (const step of steps) {
            ...
            console.log(`  ${icon} ${step.name}${step.runDir ? ` -> ${path.basename(step.runDir)}` : ''}`);
        }
        console.log('');
    };

    if (args.dryRun) {
        console.log('[DRY RUN] Pipeline would execute these steps:');
        printSteps();
        console.log(`Artifacts would be saved to: ${pipelineRun.runDir}`);
        return;
    }
    ...
    try {
        ...
        if (existsSync(ttsNamespace)) {
            const dateDirs = require('fs').readdirSync(ttsNamespace).sort().reverse();
            if (dateDirs[0]) {
    ...
    if (steps[1]!.status === 'pending') {
        ...
        try {
            ...
            if (existsSync(sfxNamespace)) {
                const dateDirs = require('fs').readdirSync(sfxNamespace).sort().reverse();
                if (dateDirs[0]) {
    ...
    if (steps[2]!.status === 'pending') {
        ...
        try {
            ...
            if (existsSync(ambianceNamespace)) {
                const dateDirs = require('fs').readdirSync(ambianceNamespace).sort().reverse();
                if (dateDirs[0]) {
    ...
    if (steps[3]!.status === 'pending') {
        ...
        try {
            ...
            if (existsSync(musicNamespace)) {
                const dateDirs = require('fs').readdirSync(musicNamespace).sort().reverse();
                if (dateDirs[0]) {
    ...
    try {
        ...
        if (existsSync(mixNamespace)) {
            const dateDirs = require('fs').readdirSync(mixNamespace).sort().reverse();
            if (dateDirs[0]) {
